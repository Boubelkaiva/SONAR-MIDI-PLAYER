...
bool BankDNAExtractor::extractToJSON(const juce::File &sf2File)
{
    ...
    // =========================
    // STABILNÍ ID BANKY (HASH)
    // =========================
    const juce::int64 bankId = computeFileHash64(sf2File);
    ...
    fluid_sfont_t *sfont = fluid_synth_get_sfont_by_id(
        synth,
    ...
    if (outFile.is_open())
    {
        ...
        while ((preset = fluid_sfont_iteration_next(sfont)))
        {
            ...
            int b = fluid_preset_get_banknum(preset);
            int p = fluid_preset_get_num(preset);

            // --- TVÁ HYBRIDNÍ LOGIKA ---
            if (instrName.containsIgnoreCase("Kit") ||
                instrName.containsIgnoreCase("Drum") ||
                instrName.containsIgnoreCase("Perc"))
            {
                b = 128;
            }
